// ============================================================
//  🤖 RIXO.IN TELEGRAM SALESBOT — GLITCH EDITION
//  ✅ No credit card | ✅ Always on 24/7 | ✅ Free forever
//  Host at: https://glitch.com
// ============================================================
...
// ============================================================
//  ✅ CONFIG — Paste values in Glitch's ".env" panel
//  (In Glitch editor → click 🔒 ".env" in left sidebar)
// ============================================================
const CONFIG = {
  ...
  LEADS_FILE: ".data/leads.json", // .data folder persists on Glitch even after edits!
};
...
const MSG = {
  ...
  price: () =>
    ...
    `🔍 Google indexing — Google पर दिखेगा\n` +
    `🔒 SSL Security (HTTPS)\n` +
    `⚡ 24 घंटे में Live — guaranteed\n` +
    `✏️ Free content updates via WhatsApp\n` +
  ...
  freelancer: () =>
    ...
    `✅ LinkedIn sharing के लिए perfect\n` +
    `✅ सिर्फ *₹499/साल*\n\n` +
    `👉 ${CONFIG.PREVIEW}`,
...
// ============================================================
//  💾 LEADS — saved to .data/leads.json (persists on Glitch)
// ============================================================
...
app.post("/webhook", async (req, res) => {
  ...
  if (body.callback_query) {
    ...
    await sendMessage(cb.message.chat.id, getMsg(cb.data, cb.from.first_name), mainMenu());
    return;
  }
...
// ============================================================
//  🏥 HEALTH CHECK — keeps Glitch app awake
// ============================================================
app.get("/", (req, res) => {
  res.send(`
    <html><head><title>Rixo Salesbot</title>
    <style>body{font-family:sans-serif;text-align:center;padding:60px;background:#f5f0ff}
    h1{color:#6c3fff}a{color:#6c3fff}</style></head>
    <body>
      <h1>🤖 Rixo.in Salesbot is Running!</h1>
      <p>Status: <strong style="color:green">✅ Online</strong></p>
      <p><a href="/leads">📊 View Leads Dashboard</a> &nbsp;|&nbsp;
         <a href="/setup">🔗 Setup Webhook</a></p>
      <p style="color:#aaa;font-size:12px">${new Date().toLocaleString('en-IN')}</p>
    </body></html>
  `);
});
...
<html><head>
  ...
  <header><h1>🤖 Rixo.in Salesbot — Leads</h1><p>Hosted on Glitch • Free Forever • No credit card</p></header>
  <div class="c">
    <div class="stats">
    ...
    <table>
      ...
      <tbody>
        ${leads.length === 0
          ? '<tr><td colspan="6"><div class="empty">📭 No leads yet! Share your bot link.</div></td></tr>'
...
// ============================================================
//  🔗 SETUP — Visit /setup ONCE to register Telegram webhook
// ============================================================
app.get("/setup", async (req, res) => {
  ...
  try {
    ...
    if (r.data.ok) {
      res.send(`
        <html><head><style>body{font-family:sans-serif;text-align:center;padding:60px;background:#f0fff4}
        h2{color:#16a34a}code{background:#f0f0f0;padding:4px 8px;border-radius:4px}</style></head>
        <body>
          <h2>✅ Webhook Set Successfully!</h2>
          <p>Telegram is now sending messages to:</p>
          <p><code>${webhookUrl}</code></p>
          <br>
          <p>🎉 Your bot is LIVE 24/7 on Glitch!</p>
    ...
    res.status(500).send(`
      <html><head><style>body{font-family:sans-serif;text-align:center;padding:60px;background:#fff5f5}
      h2{color:red}p{color:#555}</style></head>
      <body>
        <h2>❌ Setup Failed</h2>
        <p>${e.message}</p>
        <p>Check your BOT_TOKEN in the <strong>.env</strong> panel (🔒 in left sidebar)</p>
      </body></html>
    `);
  }
});
...
// ============================================================
//  ⏰ KEEP-ALIVE PING — Prevents Glitch from sleeping
//  Glitch free apps stay awake as long as they get requests.
//  This pings itself every 4 minutes automatically.
// ============================================================
...
app.listen(CONFIG.PORT, () => {
  console.log(`
  ╔══════════════════════════════════════════╗
  ║   🚀 RIXO SALESBOT — GLITCH EDITION     ║
  ╠══════════════════════════════════════════╣
  ║  1. Add BOT_TOKEN in 🔒 .env panel      ║
  ║  2. Visit /setup ONCE to go live        ║
  ║  3. Share t.me/YOUR_BOT everywhere!     ║
  ╚══════════════════════════════════════════╝`);
});
